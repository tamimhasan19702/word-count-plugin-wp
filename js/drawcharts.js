...
var drawcharts = {
  ...
  drawUsersByRoleChart: function () {
    function drawChart() {
      // build data array.
      var p_data = [["User Roles", "Total"]];
      for (let key in summary_data.user_data.avail_roles) {
        let parsed_key =
          summary_data.user_data.avail_roles[key] +
          " " +
      ...
      var chart = new google.visualization.PieChart(
        document.getElementById("summary-roles-chart")
      );
  ...
  drawLoginByRole: function () {
    function drawChart() {
      //build data array
      var p_data = [["Role", "Total"]];
      for (let key in summary_data.logins_per_role) {
        p_data.push([
          key.charAt(0).toUpperCase() + key.slice(1).toLowerCase(),
          parseInt(summary_data.logins_per_role[key]),
        ]);
      }
      ...
      var chart = new google.charts.Bar(
        document.getElementById("summary-logins-chart")
      );
